feature 'user deletes game', %Q{
  ...
  scenario 'signed in user is the creator of the team' do
    ...
    expect(page).to have_content("Week #{@game.week}")
    expect(page).to have_content(@game.away_team.name)
    expect(page).to have_content(@game.away_score)
    expect(page).to have_content(@game.home_team.name)
    expect(page).to have_content(@game.home_score)

    click_button "Delete Game"
    expect(page).to have_content("Game deleted successfully!")

    click_link "#{@team_1.location} #{@team_1.name}"
    expect(page).to_not have_content("Week #{@game.week}")
    expect(page).to_not have_content(@game.away_score)
    expect(page).to_not have_content(@game.home_score)

feature 'user sees game', %Q{
  ...
  scenario 'user view game that has stats' do
    ...
    expect(page).to have_content(@game_1.stat.away_rushes)
    expect(page).to have_content("20")
    expect(page).to have_content(@game_1.stat.away_rushing_yards)
    expect(page).to have_content("100")
    expect(page).to have_content("#{(@game_1.stat.away_rushing_yards.to_f/@game_1.stat.away_rushes)}")
    expect(page).to have_content("5")
    expect(page).to have_content(@game_1.stat.away_passes)
